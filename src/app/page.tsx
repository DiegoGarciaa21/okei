...
// Corrección de fechas locales sin timezone problemas
function parseLocalDate(dateStr: string) {
  const [y, m, d] = dateStr.split("-").map(Number);
  return new Date(y, m - 1, d);
}

// Scroll suave con offset compensado
...
export default function Home() {
  return (
    <main className="min-h-screen flex flex-col space-y-24 text-white bg-[radial-gradient(circle_at_top_left,_#ef4444_0%,_#0a0a0a_80%)] bg-no-repeat bg-cover">
...
function Hero() {
  return (
    ...
    >
      <Image src="/hero-bg.jpg" alt="Fondo Los Falsos" fill priority className="object-cover object-center z-0" />
      ...
      >
        ...
        <p className="text-white text-lg md:text-xl font-semibold mb-6 tracking-wide">
          Escucha su Último álbum y conoce a la banda
        </p>

        <div className="flex flex-col md:flex-row gap-4 justify-center">
          <button
            onClick={() => scrollToSection("conciertos")}
            className="bg-red-600 text-black font-bold px-6 py-3 rounded-full text-sm transition-all hover:scale-110 hover:-rotate-1 hover:shadow-[0_0_25px_rgba(255,0,0,0.6)]"
          >
            Ver conciertos
          </button>
          ...
          <button
            ...
            className="border border-red-500 text-red-500 px-6 py-3 rounded-full font-bold text-sm transition-all hover:bg-red-500 hover:text-black hover:scale-110 hover:rotate-1 hover:shadow-[0_0_25px_rgba(255,0,0,0.6)]"
          >
            Escuchar ahora
          </button>
        </div>
...
function QuienesSomos() {
  return (
    <motion.section
      id="quienes-somos"
      className="relative py-24 px-6 bg-[radial-gradient(ellipse_at_top,_#7f1d1d,_#000)] border-t border-zinc-800"
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center">
        <h2 className="text-5xl md:text-6xl font-extrabold text-red-500 mb-10 tracking-wide drop-shadow">
          ¿QUIENES SOMOS?
        </h2>

        <div className="w-full max-w-md mx-auto mb-10 rounded-2xl overflow-hidden shadow-[0_0_25px_rgba(255,0,0,0.5)] border border-red-600">
          <Image src="/historia.jpg" alt="La banda" width={634} height={634} className="object-cover w-full h-full" />
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto text-center mb-14">
        <h2 className="text-4xl md:text-5xl font-extrabold text-red-500 drop-shadow mb-4 tracking-wide">
          INTEGRANTES DE LA BANDA
        </h2>
        <p className="text-zinc-300 max-w-2xl mx-auto">Conoce a los músicos detrás del sonido de los falsos.</p>
      </div>
        ...
        {integrantes.map((i, idx) => (
          ...
          >
            <div className="rounded-lg overflow-hidden mb-4 border-2 border-red-600">
              <Image src={i.foto} alt={i.nombre} width={400} height={450} className="object-cover w-full h-[300px]" />
            </div>
      ...
      <div className="flex justify-center mt-12">
        <motion.div
          ...
          whileHover={{ scale: 1.05 }}
          viewport={{ once: true }}
          className="bg-zinc-900/80 backdrop-blur-sm border border-red-700 rounded-xl p-5 shadow-xl hover:shadow-red-500/40 max-w-xs cursor-pointer"
        >
          <div className="rounded-lg overflow-hidden mb-4 border-2 border-red-600">
...
function Musica() {
  return (
    <motion.section
      id="musica"
      className="relative py-40 px-6 text-center text-white bg-[radial-gradient(ellipse_at_top,_#b91c1c,_#000)] border-t border-zinc-800"
      ...
      <div className="relative z-10 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16">
        <div className="md:w-1/2 text-center md:text-left space-y-6">
          ...
          <a
            ...
            target="_blank"
            className="inline-flex items-center gap-2 bg-red-500 text-black px-8 py-3 rounded-full font-bold uppercase text-sm hover:bg-red-400 transition"
          >
          ...
          <a
            href="/discografia"
            className="inline-flex items-center gap-2 border border-red-500 text-red-500 px-8 py-3 rounded-full font-bold uppercase text-sm hover:bg-red-500 hover:text-black transition"
          >
...
function Conciertos() {
  ...
  useEffect(() => {
    if (showPast) {
      setTimeout(() => scrollToSection("conciertos"), 200);
    }
  }, [showPast]);
  ...
  const { futuros, pasados } = useMemo(() => {
    ...
    for (const c of concertsData) {
      ...
      if (f >= today) fut.push(c);
      else pas.push(c);
    }

    return {
      futuros: fut.sort((a, b) => parseLocalDate(a.fecha).getTime() - parseLocalDate(b.fecha).getTime()),
      pasados: pas.sort((a, b) => parseLocalDate(b.fecha).getTime() - parseLocalDate(a.fecha).getTime()),
    };
  }, [today]);
  ...
  return (
    <motion.section
      id="conciertos"
      className="relative py-32 px-4 text-white bg-[radial-gradient(ellipse_at_top,_#991b1b,_#000)] border-t border-zinc-900"
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 1 }}
      viewport={{ once: true }}
          ...
          {futuros.length === 0 ? (
            ...
            futuros.map((c) => (
              <motion.div
                key={c.id}
                whileHover={{ scale: 1.02 }}
                className="bg-zinc-900/90 p-6 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-md border border-red-700"
              >
                <div>
                ...
                {c.tickets && (
                  <a
                    href={c.tickets}
                    target="_blank"
                    className="bg-gradient-to-r from-red-500 to-red-700 text-white font-semibold px-4 py-2 rounded hover:scale-105 transition"
                  >
                    TICKETS
                  </a>
                )}
                ...
                {pasados.map((c) => (
                  ...
                  >
                    <div className="relative w-full aspect-[16/9]">
                      <Image src={c.image} alt={c.evento} fill className="object-cover rounded-t-xl" />
                    </div>
